...
var _ = require('underscore'),
  debugTransform = require('debug')('node-elasticsearch-sync:transform');
...
module.exports = {
  ...
  transform: function (watcher, document, callBack) {
    debugTransform('Transforming document: %s', JSON.stringify(document).trunc(200));
    ...
    if (document.index) {
      ...
      if (!document.id) {
        ...
        debugTransform('Warning: "id" field already exists, creating a suitable identifier "id" (default: using "_id") to enable document updates must be handled in user-defined transform function.');
      }
      if (watcher.transformFunction) {
        watcher.transformFunction(watcher, document, callBack);
      } else {
        debugTransform('Transform complete: %s', JSON.stringify(document).trunc(200));
        callBack(document);
      }
    }
  },
